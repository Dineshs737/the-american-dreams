...
public class techOfficerSubmitAttendanceController implements Initializable {
    ...
    void submitAttendance(ActionEvent event) {
        ...
        if (attendanceStatus.getSelectedToggle() == null) {
            showAlert(Alert.AlertType.ERROR, "Validation Error", "Please select an attendance status.");
            return;
        }
...
//        System.out.println("Selected Status: " + selectedStatus.getText());
    ...
    @FXML
    void checkStudentId(ActionEvent event) {
        if (!validateStudentId()) {
            System.out.println("Student ID validation failed.");
        } else {
            System.out.println("Student ID validation succeeded.");
        }
    }
}
